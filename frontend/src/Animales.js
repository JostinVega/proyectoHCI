...
const Animales = ({ player, onBack, onConfigClick, onProgressUpdate }) => {
  ...
  const uidToAnimalMap = {
    ...
    'a9:ce:34:9b': 'oveja',
    '79:40:ed:98': 'abeja',
    '89:ba:3c:98': 'elefante',
    '19:d5:41:98': 'iguana',
    'b9:6d:55:99': 'oso',
    '3:52:92:16': 'unicornio'
  };

  // Función para obtener el UID desde el servidor
  const fetchLastCardID = async () => {
    try {
      const response = await fetch('http://172.29.81.110:3001/data'); // Asegúrate de que esta URL sea correcta
      if (response.ok) {
  ...
  const checkAnswer = (input) => {
    ...
    const expectedAnimal = uidToAnimalMap[input]; // Obtiene el animal esperado del UID
    const currentAnimalNombre = animales[currentAnimal];
    const isRight = expectedAnimal !== undefined && expectedAnimal === currentAnimalNombre;
    setIsCorrect(isRight);

    if (isRight) {
      setUserInput(expectedAnimal);
    ...
    if (!isRight) {
      setAnimalStats((prevStats) => {
        ...
        if (!updatedStats[currentAnimalNombre]) {
          updatedStats[currentAnimalNombre] = { errors: 0, time: 0, resultado: false };
        }
        updatedStats[currentAnimalNombre].errors += 1;
        updatedStats[currentAnimalNombre].resultado = false;

        saveDetailsToDatabase({ section: 'animales', details: { [currentAnimalNombre]: updatedStats[currentAnimalNombre] } });
        console.log(`Intento incorrecto para animal ${currentAnimalNombre}:`, updatedStats[currentAnimalNombre]);
        return updatedStats;
      });
    ...
    setAnimalStats((prevStats) => {
      const updatedStats = { ...prevStats };
      if (!updatedStats[currentAnimalNombre]) {
        updatedStats[currentAnimalNombre] = { errors: 0, time: 0, resultado: true };
      }
      updatedStats[currentAnimalNombre].time = responseTime;
      updatedStats[currentAnimalNombre].resultado = true;

      saveDetailsToDatabase({ section: 'animales', details: { [currentAnimalNombre]: updatedStats[currentAnimalNombre] } });
      console.log(`Intento correcto para animal ${currentAnimalNombre}:`, updatedStats[currentAnimalNombre]);
      return updatedStats;
    });
                  ...
                  <div key={i} className="flex-1 min-w-[3rem] max-w-[4rem] mb-4">
                    <div className="relative">
                      ...
                      <div className={`relative z-10 flex flex-col items-center transform 
                                              transition-all duration-500 ${i === currentAnimal ? 'scale-110' : 'hover:scale-105'
                        }`}>
